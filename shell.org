...
Override ~ls~ if ~gls~ (the ~g~ prefixed GNU core utility installation) is available.
This could be done with an alias, but I define other aliases for ~ls~ in my [[Aliases]],
and aliases don't stack, so it's better to use a function.
:TODO: This exposes a larger problem with my aliases; I would like them each to be independent and stackable. I should switch to functions everywhere. Aliases are too restricted.
#+begin_src sh
...
I use ~mbsync~ (~isync~), ~mu~, and ~mu4e~ (the Emacs package)
to manage my emails from within Emacs.

My ~mbsync~ settings can be found in [[./mbsync.org]].
